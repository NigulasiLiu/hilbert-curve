...
//        System.out.println("BitMap_Op: " + BitMap_Op);
        ...
        System.out.println("TDSC2023_Biginteger Total update time: " + totalLoopTimeMs + " ms).");
...
////            TDSC2023_Biginteger.homomorphicEncryption.setBit(bitMap, id.intValue()); // 将id转换为长度为2^20，第id位为1的bitmap
////            BigInteger B = TDSC2023_Biginteger.homomorphicEncryption.processBitMapExistence(bitMap); // 将位图序列转换为BigInteger值
////            // 对此位置执行同态加法以产生反向操作（将1变为0）,update时，将该值视为B
////            BigInteger B_deleteId = TDSC2023_Biginteger.homomorphicEncryption.getN().subtract(B);
//
...
//            TDSC2023_Biginteger.update(pSet, W, "add", files, maxnums_w);
...
//            BigInteger BR = TDSC2023_Biginteger.Search(R_min, R_max, WQ);
...
//            TDSC2023_Biginteger.update(pSet, W, "del", files, maxnums_w);
