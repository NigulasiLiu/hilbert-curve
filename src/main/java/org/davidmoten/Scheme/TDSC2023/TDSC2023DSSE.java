...
public class TDSC2023DSSE {
    private static final String HMAC_SHA256 = "HmacSHA256";
    private static final int maxnums_w = 200;
    // 数据集路径
    private static final String FILE_PATH = "src/dataset/spatialdata.csv";
    ...
    // 构造函数
    public TDSC2023DSSE(int securityParameter) throws Exception {
...
//        System.out.println("Hilbert Index (Binary): " + hilbertBinary);
    ...
    // 更新操作
    public void update(long[] pSet, String[] W, BigInteger B, int CounterLimits) throws Exception {
...
//                String Ti = HMAC_SHA256;
        ...
        // 输出总耗时
        double totalLoopTimeMs = (System.nanoTime()-totalLoopTime) / 1_000_000.0;
        System.out.println("Total loop time: " + totalLoopTime + " ns (" + totalLoopTimeMs + " ms).");
        System.out.println("Total update time: " + (totalLoopTime+precoverTime) + " ns (" + (totalLoopTimeMs+(precoverTime/1_000_000.0)) + " ms).");
        return BR;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        TDSC2023DSSE TDSC2023Dsse = new TDSC2023DSSE(securityParameter);

        // 进行主流程测试
        while (true) {
            // 模拟从数据集中获取一项数据
            Object[] result = TDSC2023DSSE.GetRandomItem(12);
            if (result == null) {
            ...
            TDSC2023Dsse.homomorphicEncryption.setBit(bitMap, id.intValue()); // 将id转换为长度为2^20，第id位为1的bitmap
            BigInteger B = TDSC2023Dsse.homomorphicEncryption.processBitMapExistence(bitMap); // 将位图序列转换为BigInteger值

            // 对此位置执行同态加法以产生反向操作（将1变为0）,update时，将该值视为B
            BigInteger B_deleteId = TDSC2023Dsse.homomorphicEncryption.getN().subtract(B);
            ...
            TDSC2023Dsse.update(pSet, W, B, maxnums_w);
            ...
            BigInteger BR = TDSC2023Dsse.Search(R_min, R_max, WQ);
            ...
            TDSC2023Dsse.update(pSet, W, B_deleteId, maxnums_w);
