...
//
//    // 构造函数
//    public Ours(int securityParameter) throws Exception {
...
//        Ours ours = new Ours(securityParameter);
//
//        // 进行主流程测试
//        while (true) {
//            // 模拟从数据集中获取一项数据
//            Object[] result = Ours.GetRandomItem(12);
//            if (result == null) {
...
//            ours.homomorphicEncryption.setBit(bitMap, id.intValue()); // 将id转换为长度为2^20，第id位为1的bitmap
//            BigInteger B = ours.homomorphicEncryption.processBitMapExistence(bitMap); // 将位图序列转换为BigInteger值
//
//            // 对此位置执行同态加法以产生反向操作（将1变为0）,update时，将该值视为B
//            BigInteger B_deleteId = ours.homomorphicEncryption.getN().subtract(B);
//
...
//            ours.update(pSet, W, B, maxnums_w);
...
//            BigInteger BR = ours.Search(R_min, R_max, WQ);
...
//            ours.update(pSet, W, B_deleteId, maxnums_w);
