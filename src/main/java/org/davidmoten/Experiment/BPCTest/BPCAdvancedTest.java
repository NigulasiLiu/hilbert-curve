...
public class BPCAdvancedTest {

    private HilbertCurve hilbertCurve;
    private int order;

    // 构造函数初始化HilbertCurve实例
    public BPCAdvancedTest(int order) {
        this.order = order;
...
//        double[] sizeRatios = {0.02, 0.04, 0.06, 0.08, 0.10, 0.2};
        double[] sizeRatios = {0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14, 0.16, 0.18, 0.2};
        ...
        for (int hilbertOrder : hilbertOrders) {
            ...
            for (int i = 0; i < sizeRatios.length - 1; i++) {
                    ...
                    Callable<Void> task = () -> {
                        ...
                        // 进行5次实验
                        for (int rectNum = 0; rectNum < 100; rectNum++) {
                            ...
                            // 获取每个线程的文件名
                            String fileName = String.format("src/dataset/BPCTestResult/100Region_%d_%.2f_%.2f_result.csv", hilbertOrder, ratio1, ratio2);
                            // 写入到该文件
