...
//        int[] updateItems = new int[updatetimes];
        ...
        for (int batch = 0; batch < updatetimes / searchbatch; batch++) {
            for (int i = 0; i < searchbatch; i++) {
...
//            updateItems[i] = randomIndex;
                ...
                if ((i + 1) % batchSize == 0) {
                    //System.gc(); // 执行垃圾回收
                    System.out.println("Completed batch " + (i + 1) / batchSize + " of searchbatch.");
                }
            }
            int div = 100;
            searchtimes = 1;
            ...
            int xlen = i1;
            int ylen = i1;
            for (int xstart = 0; xstart < edgeLength; xstart += edgeLength * searchEdgeLengthPer / div) {
                int ystart = xstart;
                BigInteger[][] matrixToSearch = generateHilbertMatrix(spqs.hilbertCurve, xstart, ystart, xlen, ylen);
